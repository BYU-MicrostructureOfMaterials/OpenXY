...
for j = 1:length(Grains)
    ...
    for subgNum = 1:size(subGrains,1)
        ...
        for i = 2:length(path)
            ...
            Ftemp = SwitchF(RefImage,ScanImage,g(:,:,RefInd),eye(3),ScanInd,Settings,Settings.Phase{ScanInd},RefInd);%try switch func
            R = poldec(Ftemp);
            Rtot = Rtot*R;
            RefImage = ScanImage;
        end
...
for i = 2:length(Inds)
    ...
    %F = CalcF(RefImage,ScanImage,g(:,:,RefInd),eye(3),ScanInd,Settings,Settings.Phase{ScanInd},RefInd);
    F = SwitchF(RefImage,ScanImage,g(:,:,RefInd),eye(3),ScanInd,Settings,Settings.Phase{ScanInd},RefInd); %try switch
    R = poldec(F);
    Rtot = Rtot*R;
    g_new(:,:,i) = R'*g_new(:,:,i-1);
...
ScanImage = ReadEBSDImage(Settings.ImageNamesList{ScanInd});
%F_test = CalcF(RefImage,ScanImage,g(:,:,Ind),eye(3),Inds(end),Settings,Settings.Phase{Inds(end)},Ind);
F_test = SwitchF(RefImage,ScanImage,g(:,:,Ind),eye(3),Inds(end),Settings,Settings.Phase{Inds(end)},Ind);
R = poldec(F_test);
g_test = R'*g(:,:,RefInd);
...
F = SwitchF(RefImage,ScanImage,g(:,:,Ind),eye(3),Inds(end),Settings,Settings.Phase{Inds(end)},Ind);
R = poldec(F);
g_subg = R'*g(:,:,RefInd);
R2 = (g_subg*g(:,:,RefInd)')';
