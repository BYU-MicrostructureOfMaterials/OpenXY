...
% end
% if ~exist(fullfile(EMdataPath,sprintf('%s_EBSDmaster.h5',Material)),'file')
%     masterfile = (sprintf('MCoutput_%s.h5',Material));
% else
%     masterfile = (sprintf('MCoutput_%s.h5',Material)); 
% end
...
%EMdataPath2 = strcat(EMdataPath, 'EMdata/');
...
anglefilepath = fullfile(EMdataPath, anglefile);
%this ^^ is the original line here
%inputfile = fullfile(EMdataPath,'EMdata/', ['OpenXY_' num2str(ImageInd) '.nml']);
...
%fid=fopen(fullfile(EMdataPath,'EMdata\', anglefile),'w');
...
fprintf(fid,formatString,L,thetac,delta,numsx,numsy,xpc,ypc,...
    alphaBD,energymin,energymax,includebackground,anglefile,...
    eulerconvention,masterfile,datafile,bitdepth,...
    beamcurrent,dwelltime,binning,applyDeformation,Ftensor,...
    scalingmode,gammavalue,maskpattern,floor(numsx/2),nthreads);
...
%commented out because I commented out line 215
pic=zeros(numsx,numsy);
pic(:,:)=data1(:,:,1); %commented out because I commented out data1
