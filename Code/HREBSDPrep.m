...
if strcmp(Settings.HROIMMethod,'Dynamic Simulated')
    ...
    if Settings.SinglePattern
        ...
        PC = [Settings.XStar; Settings.YStar; Settings.ZStar];%this could be wrong, I'm to too familiar with EMsoft integration, so it may need to be changed. ZRC 4/14/27
        ...
        [F1,~,~] = CalcF(RefImage,ScanImage,gr,eye(3),ImageInd,...
            curMaterial,Settings.RefImageInd,PC,Setting.roixc,...
            Settings.roiyc,Settings.ROIFilter,ROISize,standev,...
            Settings.SampleTilt,pixsize,elevang,calcMI,0,method);
        for iq=1:3
