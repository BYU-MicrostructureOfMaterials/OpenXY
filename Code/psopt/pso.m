...
state.ParticleInertia = options.InitialParticleInertia ; % Initial inertia OKJ
...
for k = 1:options.Generations
    ...
    if strcmpi(options.Vectorized,'off')
        if strcmpi(options.Parallel,'off')
            for i = setdiff(1:n,find(state.OutOfBounds))
                state.Score(i) = fitnessfcn(state.Population(i,:));
                if any(isnan(state.Score(i)))
                   'stop here'; 
                end
            end % for i
        elseif strcmpi(options.Parallel,'on')
